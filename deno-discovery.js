...
function async_sleep(microsecond) {
...
function app_register(req, pu) {
    if (pu.decoded_query_map.get("protocol") === "http") {
        ...
        if (common_name === undefined) {
            return err400("No &common-name parameter (required)");
        }
        if (host === undefined) {
            return err400("No &host parameter (required)");
        }
        if (ttl_text && /^[0-9]+$/.test(ttl_text) === false) {
            return err400("Parameter &ttl malformed (optional, but must be decimal number if defined)");
        }
        const ttl_seconds = Math.max(60, Math.min(86400, parseInt(ttl_text || "300", 10)));
        const record_object = {
            protocol: "http",
            host,
            ttl: Date.now() + ttl_seconds * 1000,
            health: {
                last_checked: Date.now(),
                last_healthy: Date.now(),
                alive: true,
                ack_latency: 0
            }
        };
        ...
        if (existing) {
            existing.set(host, record_object);
        } else {
            memory.set(common_name, new Map([
                [
                    host,
                    record_object
                ]
...
function app_discover(req, pu) {
    const common_name = pu.decoded_query_map.get("common-name");
    if (common_name === undefined) {
        return {
            status: 400,
            body: "No &common-name parameter (required)"
        };
    }
    ...
    if (dump === "1") {
        return {
            body: readerFromStreamReader(readableStreamFromIterable(record_json_iter_serialize(common_name, record.entries())).getReader())
        };
    }
    const nth_text = pu.decoded_query_map.get("nth");
    if (nth_text !== undefined && /^[0-9]+/.test(nth_text) !== true) {
        return {
            status: 400,
            body: "Parameter &nth malformed, please supply only decimal characters (^[0-9]+$). Remember this parameter is optional."
        };
    }
    const nth = nth_text === undefined ? null : parseInt(nth_text);
    const entires = [
        ...record.entries()
    ].filter((entry)=>entry[1].health.alive
    );
    while(entires.length > 0){
        ...
        if (host === undefined || single_record === undefined) {
            return {
                status: 500,
                body: "500.1 unexpected"
            };
        }
        if (single_record.ttl < Date.now()) {
            record.delete(host);
            continue;
        } else {
            return {
                status: 200,
                body: single_record_serialize(common_name, host, single_record)
            };
        }
    }
    return {
...
async function* record_json_iter_serialize(common_name, record_iter) {
    for (const record of record_iter){
        yield single_record_serialize(common_name, record[0], record[1]);
    }
    return;
}
function single_record_serialize(common_name, host, single_record) {
    ...
    return encoder1.encode(json_string);
}
function app_dump(req, pu) {
    return {
        body: JSON.stringify([
            ...memory.entries()
        ].map(([k, v])=>[
...
function app_ack(req, pu) {
    return {
        status: 200,
        body: pu.decoded_fragments[1]
    };
}
async function health_check(common_name, host, endpoint_record) {
    if (endpoint_record.protocol === "http") {
        ...
        try {
            res = await Promise.race([
                fetch(`http://${endpoint_record.host}/ack/${random_text}`, {
...
async function check_all_serial() {
...
console.log(`http server ${common_name_header1[1]} is running. Come at http://localhost:9999/`);
    ...
    try {
        for await (const request of server){
            ...
            try {
                ...
                if (response.headers) {
                    response.headers.set(common_name_header1[0], common_name_header1[1]);
                } else {
...
export { common_name_header1 as common_name_header };
